<?php
add_action('init', 'register_lab_type');
function register_lab_type() {
	$labels = array(
		'name'               => _x( 'Labs', 'post type general name', 'thcPlugin' ),
		'singular_name'      => _x( 'Lab', 'post type singular name', 'thcPlugin' ),
		'menu_name'          => _x( 'Labs', 'admin menu', 'thcPlugin' ),
		'name_admin_bar'     => _x( 'Lab', 'add new on admin bar', 'thcPlugin' ),
		'add_new'            => _x( 'Add New', 'lab', 'thcPlugin' ),
		'add_new_item'       => __( 'Add New Lab', 'thcPlugin' ),
		'new_item'           => __( 'New Lab', 'thcPlugin' ),
		'edit_item'          => __( 'Edit Lab', 'thcPlugin' ),
		'view_item'          => __( 'View Lab', 'thcPlugin' ),
		'all_items'          => __( 'All Labs', 'thcPlugin' ),
		'search_items'       => __( 'Search Labs', 'thcPlugin' ),
		'parent_item_colon'  => __( 'Parent Labs:', 'thcPlugin' ),
		'not_found'          => __( 'No labs found.', 'thcPlugin' ),
		'not_found_in_trash' => __( 'No labs found in Trash.', 'thcPlugin' )
	);
...
add_action( 'add_meta_boxes_lab', 'lab_meta_boxes' );
function lab_meta_boxes(){
	add_meta_box('lab-data-box', 'Lab Data', 'lab_data_box_html', 'lab', $context='advanced', $priority='default', $callback_args=null);
	add_meta_box('lab-schedule-box', 'Hours Opened', 'lab_schedulebox_html', 'lab', $context='advanced', $priority='default', $callback_args=null);
}
function lab_data_box_html($post){
	global $wpdb;
	if($post && $post->ID){$ddata = get_all_data($post->ID);}
	if(current_user_can('administrator')){
		$blog_id = get_current_blog_id();
		$roles = array('administrator', 'lab');
...
function lab_schedulebox_html($post){
...
add_action('save_post', 'lab_save_post', 10, 2);
function lab_save_post($pid, $post){
...
add_action( 'delete_post', 'lab_delete' );
function lab_delete($pid){
	$post = get_post($pid);
	if ( $post->post_type != 'lab' ) return;
